...
class StudentForm(forms.Form):
    name=forms.CharField(max_length=100,validators=[check_for_a,validators.MinLengthValidator(5)])
   ...
   # def clean_age(self):
    #    a=self.cleaned_data['age']
     #   if a>18:
     #       raise forms.ValidationError('age is below 18')


    def clean_botcatcher(self):
        bot=self.cleaned_data['botcatcher']
        if len(bot)>0:
            raise forms.ValidationError('data is not correct')